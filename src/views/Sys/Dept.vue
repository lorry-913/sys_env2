<template>
  <div class="page-container">
    ...
    <!-- 增加页面 -->
    <el-dialog title="新增" width="40%" :visible.sync="addDialogVisible" :close-on-click-modal="false">
      ...
      <span slot="footer" class="dialog-footer">
        <el-button :size="size"  @click="addDialogVisible = false">{{$t('action.cancel')}}</el-button>
    ...
    <el-dialog title="编辑" width="40%" :visible.sync="editDialogVisible" :close-on-click-modal="false">
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button :size="size"  type="primary" @click="editSubmitForm()">{{$t('action.comfirm')}}</el-button>
      </span>
    </el-dialog>
  </div>
</template>
    ...
    methods: {
      ...
      handleDelete (row) {
        this.$confirm('确认删除选中记录吗？', '提示', {
          ...
          this.$api.dept.deptdelete({"id":this.selectionsDept.id}).then( res => {
            if(res.code==0){
              this.findTreeData()
